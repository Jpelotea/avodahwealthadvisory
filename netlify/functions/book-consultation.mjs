...
const previewBooking = (leadId, start) => {
  const duration = getConsultationConfig().duration_minutes;
  const startDate = new Date(start);
  return { ok: true, code: "PREVIEW_BOOKING_SIMULATED", preview_simulated: true, already_booked: false, booking_id: `AWA-PREVIEW-${leadId.slice(0, 8).toUpperCase()}`, start: startDate.toISOString(), end: new Date(startDate.getTime() + duration * 60000).toISOString(), meet_link: "", management_url: "", reconciliation_required: false, calendar_created: false, meet_created: false, email_sent: false, operations_hub_synced: false, warnings: ["Preview simulation only. No calendar event, email, Google Meet, or Operations Hub record was created."] };
};
...
export default async (request) => {
  ...
  try {
    ...
    if (isPreviewSimulation()) return json(previewBooking(leadId, start));
    ...
    if (!result.ok) { const status = appsScriptStatus(result); console.error("Booking Apps Script response:", JSON.stringify({ http_status: result.http_status || response.status, code: result.code || "BOOKING_FAILED", failed_action: result.failed_action || "booking", retryable: Boolean(result.retryable) })); return json(failure(result, status), status); }
    return json({ ok: true, code: result.code || "BOOKING_CREATED", preview_simulated: false, already_booked: Boolean(result.already_booked), booking_id: result.booking_id || "", start: result.start || "", end: result.end || "", meet_link: result.meet_link || "", management_url: result.management_url || "", reconciliation_required: Boolean(result.reconciliation_required), calendar_created: result.calendar_created !== false, meet_created: result.meet_created !== false && Boolean(result.meet_link), email_sent: result.email_sent === true, operations_hub_synced: result.operations_hub_synced !== false && !result.reconciliation_required, warnings: Array.isArray(result.warnings) ? result.warnings.slice(0, 10) : [] });
  } catch (error) {
