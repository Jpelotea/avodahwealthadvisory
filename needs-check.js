...
const paths = {
  ...
  loan: {label:"Loan Advisory & Preparation",copy:"Loan preparation support can help you understand likely requirements, organize documents, and compare relevant provider pathways. Approval remains with the provider.",link:"/before-applying-for-a-loan.html",questions:[{name:"loan_type",text:"What type of loan are you considering?",options:["Personal loan","Business loan","Auto loan","OFW or seafarer loan","Government employee loan","Not sure"]},{name:"loan_timing",text:"When do you expect to apply?",options:["As soon as possible","Within 1–3 months","Later this year","Just exploring"]}]},
  business: {label:"Business & Legacy Planning",copy:"A business and legacy review can help identify continuity, employee, estate, and succession priorities before exploring specific solutions.",link:"/business-protection-planning.html",questions:[{name:"business_need",text:"What would you like to address first?",options:["Business continuity","Key-person or employee protection","Estate tax preparation","Succession or legacy planning"]},{name:"business_stage",text:"What best describes your stage?",options:["New business","Growing business","Established business","Family-owned business"]}]},
  nonlife: {label:"Non-Life & General Protection",copy:"A general protection review can help identify suitable coverage categories for a vehicle, property, business asset, or trip.",link:"/non-life-insurance-assistance.html",questions:[{name:"asset_to_protect",text:"What needs protection?",options:["Vehicle","Home or property","Business assets","Upcoming trip","Other"]},{name:"coverage_status",text:"Do you currently have coverage?",options:["No coverage yet","Coverage is expiring","I want to compare options","I am not sure"]}]},
  travel: {label:"Travel & Ticketing Assistance",copy:"Travel assistance is the most relevant route for flight planning, visa guidance, passport appointment support, tours, and travel protection.",link:"/travel-ticketing-assistance.html",questions:[{name:"travel_need",text:"What support do you need?",options:["Flight ticketing","Visa guidance","Passport appointment guidance","Tour planning","Travel insurance"]},{name:"travel_timing",text:"When is your expected travel?",options:["Within 30 days","1–3 months","3–6 months","No fixed date yet"]}]}
};
...
function showStep(number){currentStep=number;steps.forEach(s=>s.classList.toggle("is-active",Number(s.dataset.step)===number));const percent=Math.round(number/3*100);stepLabel.textContent=`Step ${number} of 3`;progressPercent.textContent=`${percent}%`;progressTrack.dataset.step=String(number);progressTrack.setAttribute("aria-valuenow",String(number));backButton.hidden=number===1;nextButton.hidden=number===3;submitButton.hidden=number!==3;document.querySelector(".intake-card").scrollIntoView({behavior:"smooth",block:"start"});}
...
form.addEventListener("change",event=>{if(event.target.name==="primary_need"){trackEvent("need_selected",{primary_need:event.target.value});}if(event.target.name==="book_consultation"){const wantsContact=event.target.value==="Yes";contactFields.hidden=!wantsContact;contactFields.querySelectorAll("input,select").forEach(field=>{field.required=wantsContact&&["full_name","mobile_number","location","preferred_contact_method","processing_consent"].includes(field.name);});if(wantsContact){trackEvent("consultation_requested",{primary_need:selectedNeed(),service_path:document.querySelector("#service-path").value});}}});
form.addEventListener("submit",async event=>{event.preventDefault();const choice=form.querySelector('input[name="book_consultation"]:checked');const error=document.querySelector("#submit-error");if(!choice){error.textContent="Please tell us whether you would like a consultation.";return;}if(!form.checkValidity()){error.textContent="Please complete the required contact and consent fields.";form.reportValidity();return;}error.textContent="";submitButton.disabled=true;submitButton.textContent="Sending…";try{const submissionId=createLeadSubmissionId();form.elements.lead_submission_id.value=submissionId;const data=new FormData(form);const response=await fetch("/",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:new URLSearchParams(data).toString()});if(!response.ok)throw new Error("Submission failed");const eventParameters={primary_need:selectedNeed(),service_path:document.querySelector("#service-path").value,consultation_requested:choice.value==="Yes"?"yes":"no"};trackEvent("form_submitted",eventParameters);if(choice.value==="Yes"){trackEvent("generate_lead",eventParameters);}form.hidden=true;document.querySelector(".progress-wrap").hidden=true;const confirmation=document.querySelector("#confirmation");confirmation.hidden=false;document.querySelector("#confirmation-title").textContent=choice.value==="Yes"?"Your consultation request has been received.":"Your needs check is complete.";document.querySelector("#confirmation-copy").textContent=choice.value==="Yes"?"Thank you. You may choose an available consultation time below, or wait for Avodah to contact you.":"Thank you for using the Avodah needs check. You can explore the suggested service path now and contact Avodah whenever you are ready.";if(choice.value==="Yes"&&window.AvodahBooking){window.AvodahBooking.activate(submissionId);}confirmation.focus();}catch{error.textContent="We could not submit your answers just now. Please try again in a moment.";}finally{submitButton.disabled=false;submitButton.textContent="Finish";}});
